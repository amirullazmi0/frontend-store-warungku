...
const TransactionsSection: React.FC = () => {
	...
	const fetchTransactions = async () => {
		setLoading(true);
		try {
			const response = await axios.get(`${API_URL}/transactions/getTransactionsByUser?accessToken=${accessToken}&status=${tabValue}`, {
						...
						{transactions.map(tx => (
							<TableRow key={tx.transaction_id}>
								...
								<TableCell>
									<Button
										variant='outlined'
										onClick={() => handleUpdateStatus(tx.transaction_id, statusUpdate)}
										disabled={tx.invoice?.status === 'CANCELLED' || tx.invoice?.status === 'DONE'}>
										Update Pesanan
									</Button>
								</TableCell>
								<TableCell>
									<Button
										variant='outlined'
										onClick={() => handleUpdateStatus(tx.transaction_id, 'CANCELLED')}
										disabled={tx.invoice?.status === 'CANCELLED' || tx.invoice?.status === 'DONE'}>
										Batalkan
									</Button>
								</TableCell>
	...
	return (
		<Box sx={{ width: '100%', minHeight: '100vh', padding: 2 }}>
			...
			<TabContext value={tabValue}>
				...
				<TabPanel value='CANCELLED'>{renderTransactionTable('CANCELLED')}</TabPanel>
